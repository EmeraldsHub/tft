...
import { ensureAveragePlacement, getLiveGameStatus, getRankedInfo } from "@/lib/riotData";
...
export async function GET(
  ...
  const { data } = await supabaseAdmin
    ...
    .select(
      "id, riot_id, region, slug, is_active, puuid, summoner_id, avg_placement_10, avg_placement_updated_at"
    )
    .eq("slug", params.slug)
    .maybeSingle();
